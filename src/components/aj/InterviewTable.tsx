...
import { getInterviewsByFaculty, getUsersById, getApplicationsById, getJobOffersById, getJobOffersUnapprovedById } from '../../utils/apiCalls';
...
const InterviewTable: React.FC<{ faculty: string }> = ({ faculty }) => {
  const [data, setData] = useState<DataType[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const interviews = await getInterviewsByFaculty(faculty);

        // Fetch additional data for each interview
        const enrichedData = await Promise.all(
          interviews.map(async (interview: DataType) => {
            const user = await getUsersById(interview.studentId);
            const company = await getUsersById(interview.companyId);
            ...
            return {
              ...
              companyName: company?.username, // Add username to the interview data
